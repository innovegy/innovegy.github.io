...
function disenosInnovegy(data){
    ...
    data.feed.entry.map(
        function(linea,index){
            inner1+=`<div class="mbr-gallery-item mbr-gallery-item--p1" data-video-url="false" data-tags="Increíble"><div href="#lb-gallery1-15" data-slide-to="${index}" data-toggle="modal" class="gal"><img src="${linea.gsx$imgurl.$t}" alt="${linea.gsx$imgalt.$t}" title="${linea.gsx$titulo.$t}"><span class="icon-focus"></span></div><div><a class="display-5" href="${linea.gsx$pageurl.$t}" target="_blank">${linea.gsx$titulo.$t}</a></div></div>`
